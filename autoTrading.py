...
handler = RotatingFileHandler(LOG_DIR / "trade.log",
                              maxBytes=3_000_000,
...
def get_fear_and_greed() -> Optional[int]:
    ...
    try:
        val = int(requests.get(
            "https://api.alternative.me/fng/?limit=1", timeout=10
        ).json()["data"][0]["value"])
        FNG_CACHE.update(ts=time.time(), value=val); return val
...
def save_account(conn, acc: Account):
    conn.execute("UPDATE account SET krw=?, btc=?, avg_price=? WHERE id=1",
                 (acc.krw, acc.btc, acc.avg_price))

@with_db
def log_indicator(conn, d: dict):
    conn.execute("""INSERT INTO indicator_log
        (ts,sma,atr,vol20,macd_diff,price,fear_greed)
        VALUES(:ts,:sma,:atr,:vol20,:macd_diff,:price,:fear_greed)""", d)
...
def log_trade(conn, d: dict):
    conn.execute("""INSERT INTO trade_log
        ...
        VALUES(:ts,:decision,:percentage,:pattern,:reason,
               :btc_balance,:krw_balance,:avg_price,:price,:mode,:reflection)""", d)
...
def load_cached_ohlcv():
    ...
    try:
        ...
        if time.time()-raw["ts"]>CACHE_TTL: return None
        df=pd.DataFrame(raw["ohlcv"]); df.index=pd.to_datetime(df.index); return df
    except Exception: os.remove(CACHE_FILE); return None

def save_cached_ohlcv(df):
    cp=df.copy(); cp.index=cp.index.astype(str)
...
def fetch_direct():
    ...
    r=requests.get(f"https://api.upbit.com/v1/candles/minutes/{unit}",
                   params={"market":TICKER,"count":100},timeout=5)
...
def safe_ohlcv():
    try:
        ...
        logger.warning("pyupbit error %s", e); df=None
...
def sync_account_upbit() -> Account:
    try:
        ...
        g=lambda cur,f='balance': float(next((b.get(f,'0') for b in bal if b['currency']==cur),'0') or 0)
        return Account(g('KRW'), g('BTC'), g('BTC','avg_buy_price'))
    except Exception as e:
        logger.warning("balance sync error – %s", e); return Account(0,0,0)
...
def ask_ai_reflection(df: pd.DataFrame, fear_idx: int) -> Optional[str]:
    ...
    prompt=(f"You are a crypto trading coach.\nRecent trades: {df.to_json(orient='records')}\n"
    ...
    try:
        ...
        logger.warning("OpenAI reflection error – %s", e); return None
...
def ai_trading():
    ...
    if df is None or df.empty: df = safe_ohlcv()
    if df is None or df.empty:
        logger.error("OHLCV fetch failed"); return

    save_cached_ohlcv(df); df = calc_indicators(df)
    last = df.iloc[-1]; ts_end = last.name.floor("4h")

    if sqlite3.connect(DB_FILE).execute(
        "SELECT 1 FROM indicator_log WHERE ts>=?", (ts_end.timestamp(),)
    ).fetchone():
        logger.info("Candle %s already processed", ts_end); return

    fear = get_fear_and_greed() or 0
    price = float(last["close"]); macd = float(last["macd_diff"])

    # 먼지 간주: 잔량 0 처리 + 평단 리셋
    if acc.btc * price < MIN_ORDER_KRW:
        acc.btc = 0; acc.avg_price = 0
    ...
    if buy:
        ...
        reason = ("Stop-loss −6%" if stop_loss else
                  "Take-profit +5%" if take_profit else
                  "FG>=70 & MACD<0")
    else:
        decision, pct, reason = "hold", 0, "No signal"

    if buy and acc.krw < MIN_ORDER_KRW*2: pct = 100.0

    rows=sqlite3.connect(DB_FILE).execute(
        "SELECT decision,percentage FROM trade_log ORDER BY id DESC LIMIT 20").fetchall()
    wins=sum(1 for d,p in rows if d=="buy" and p>0); win_rate=wins/len(rows) if rows else 0.5
    dyn_cap=BUY_PCT if len(rows)<5 else 0.005 if win_rate<0.4 else 0.015 if win_rate<0.6 else BASE_RISK
    if decision=="buy": pct=min(pct,dyn_cap*100)

    krw, btc, avg = acc.krw, acc.btc, acc.avg_price; executed=False
    if decision=="buy" and pct>0:
        amt=krw*pct/100
        if not(LIVE_MODE and amt<MIN_ORDER_KRW):
            executed=True
            if LIVE_MODE: UPBIT.buy_market_order(TICKER, amt)
            else:
                qty=amt/price; krw-=amt; btc+=qty
                avg=(avg*acc.btc+amt)/btc if btc else price

    elif decision=="sell" and btc>0:
        qty=btc*pct/100; value=qty*price
        if not(LIVE_MODE and value<MIN_ORDER_KRW):
            executed=True
            if LIVE_MODE: UPBIT.sell_market_order(TICKER, qty)
            else: krw+=value; btc-=qty

    if LIVE_MODE and executed:
        acc = sync_account_upbit(); krw, btc, avg = acc.krw, acc.btc, acc.avg_price

    reflection = ask_ai_reflection(get_recent_trades(20), fear) or ""

    save_account(Account(krw, btc, avg))
    ts=time.time()
    log_indicator({"ts":ts,"sma":float(last["sma"]),"atr":float(last["atr"]),
                   "vol20":float(last["vol20"]),"macd_diff":macd,
                   "price":price,"fear_greed":fear})
    log_trade({"ts":ts,"decision":decision,"percentage":pct,"pattern":None,
               "reason":reason,"btc_balance":btc,"krw_balance":krw,
               "avg_price":avg,"price":price,
               "mode":"live" if LIVE_MODE else "virtual","reflection":reflection})
    logger.info("Executed=%s pct=%.2f mode=%s", executed, pct,
